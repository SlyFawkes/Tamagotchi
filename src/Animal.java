...
private boolean dead;
	...
	public Animal (String aName, int aHealth, int aMood, int aHunger, int aHygiene, int aEnergy, char aGender) { 
	...
	public int Age() { //This is called when the pet is over a day old
		...
		Calendar calcAge = new GregorianCalendar(birthDate.get(1), birthDate.get(2), birthDate.get(5));
		
		while (calcAge.equals(currentDate) == false) { //adds days onto brirthDate until it is equal to the current date giving age in days 
			
			calcAge.add(Calendar.DAY_OF_MONTH, 1); //adds a day to birthDate
